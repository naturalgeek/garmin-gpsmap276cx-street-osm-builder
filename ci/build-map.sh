...
# Vehicle top-speed limiter. VAN85=<class> caps routing road-speed at that class
# (4 = <=85 km/h, 5 = <=100). Writes inc/speedcap (included by the style's lines
# finalize). Only LOWERS fast/unlimited roads; posted-below-cap roads unchanged.
SPEEDCAP="$STYLE_DIR/inc/speedcap"
if [ -n "${VAN85:-}" ]; then
  echo "[build] capping routing speed at road-speed class $VAN85 (VAN85)"
  {
    echo "# generated by build-map.sh (VAN85=$VAN85) — do not edit"
    echo "highway=* & mkgmap:road-speed-max > $VAN85 { set mkgmap:road-speed-max = $VAN85 }"
    echo "highway=* & mkgmap:road-speed-max!=* { set mkgmap:road-speed-max = $VAN85 }"
  } > "$SPEEDCAP"
else
  echo "# no speed cap (VAN85 unset)" > "$SPEEDCAP"
fi
