...
#
# Build a routable Garmin gmapsupp.img for a GPSMAP 276Cx using the
# Frikart driving-tuned mkgmap style + TYP (FID=981) from frikart.no,
# with a search-optimised index. Companion to build-germany-jbm.sh.
#
# Frikart files (style + TYP source) are expected to be staged in
# /root/frikart-build/ on the build host (see staging step below).
#
...
: "${BOUNDS_SOURCE:=local}"
: "${MAPNAME:=73240001}"
# Frikart's TYP source. Single variant: OSM_TYP_ID981 (FID=981).
: "${TYP_NAME:=OSM_TYP_ID981}"
# Frikart staging dir on the build host — must contain points, polygons,
# lines, OSM_TYP_ID981.txt. Populated by hand from frikart.no/dok/*.
: "${FRIKART_STAGING:=/root/frikart-build}"
...
ARGS_FILE_SRC="$(cd "$(dirname "$0")" && pwd)/mkgmap-frikart.args"
...
# ---------------------------------------------------------------------------
# 2. Frikart style (staged manually — files from frikart.no/dok/)
# ---------------------------------------------------------------------------
STYLE_DIR_NAME='Style - Frikart'
STYLE_PATH="$WORK/style/$STYLE_DIR_NAME"
if [[ ! -d "$STYLE_PATH" ]]; then
    log "Staging Frikart style from $FRIKART_STAGING"
    mkdir -p "$STYLE_PATH"
    for f in points polygons lines OSM_TYP_ID981.txt; do
        if [[ ! -f "$FRIKART_STAGING/$f" ]]; then
            echo "ERROR: missing $FRIKART_STAGING/$f. Stage Frikart files first." >&2
            exit 1
        fi
        cp "$FRIKART_STAGING/$f" "$STYLE_PATH/"
    done
    # Provide an options file (mkgmap's default would be too coarse for
    # handheld zoom mapping). Use Jbm's known-good levels mapping.
    cat > "$STYLE_PATH/options" <<'OPT_EOF'
levels=0:24, 1:22, 2:20, 3:18, 4:17
overview-levels=5:16, 6:15, 7:14, 8:13
OPT_EOF
    cat > "$STYLE_PATH/info" <<'INFO_EOF'
summary: Frikart driving style adapted for 276Cx
description: Frikart's clean driving-oriented OSM style, originally by Sverre Didriksen (CC-BY-SA / GPL via OSM)
INFO_EOF
fi
[[ -d "$STYLE_PATH" ]] || { echo "ERROR: style not found at $STYLE_PATH" >&2; exit 1; }

# Locate TYP source (Frikart only ships one — OSM_TYP_ID981).
